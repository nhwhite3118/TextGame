...
def compare(*options):
	...
	if(user in options):
		return user
	return False
def say(words,wait=-1):
	print(words)
	print("...")
	if(wait == -1):		#Leave the option for custom timing for dramatic effect
		wait = (len(words)/17)+1	#Average reading speed is ~16.6 char/s 
	time.sleep(wait)
...
effort to detect the escape key, so its not going to do anything. Sorry, not sorry.""",0.5)
say("""However, I am sorry that there's no saving at the moment, I'll add it once there's something worth saving. Until then, enjoy this placeholder text.""",0.5)
...
name = input(">"+name+":")
say("Ok, "+name+", strap in that seat belt and get ready!",0)
time.sleep(5)
say("***COUGH***",0.5)
say("***COUGH***",0.5)
say("AhahahaHERM",1)
say("""Nananananananananana, intro song!""",0)
time.sleep(2)
say("Cut to huge space ships",0)
time.sleep(2)
say("Cut to a dragon flying over New Zealand",0)
time.sleep(2)
